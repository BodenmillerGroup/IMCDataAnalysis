...
sce <- readSCEfromTXT("data/compensation/IMMUcan_Batch20201113_compensation_corrected/") 
...
write.csv(cur_sm, "data/spillover_matrix.csv")
```
...
In the current setting, the channel ordering already matches the channel order
used by the `steinbock` framework: channels are ordered by mass.

The spillover matrix can now be saved as .tiff image for the use with `CellProfiler`.

```{r save-sm-as-tiff, message=FALSE}
library(tiff)
writeTIFF(cur_sm, "data/spillover_matrix.tiff", 
          bits.per.sample = 32)
```
...
For demonstration purposes, we read in a multi-channel image.
The `channelNames` need to be set according to their order.

```{r read-in-image, message=FALSE}
#library(cytomapper)

#img <- loadImages("data/Figure_4/IMC_image/analysis/imc_example_image_a0_full.tiff")
#channelNames(img) <- ordered_chs$V1
#img
```

We can now use the correclty ordered spillover matrix to compensate the 
multi-channel image:

```{r image-compensation, message = FALSE}
#img_comp <- compImage(img, adapted_sm)
```
