...
```{r batch-correction-harmony, message=FALSE, warning=FALSE, eval=FALSE}
library(harmony)
...
```{r visualizing-batch-correction-harmony-1, message=FALSE, warning=FALSE, fig.height=3, eval=FALSE}
# visualize patient id 
p1 <- dittoDimPlot(spe, var = "patient_id", 
                   reduction.use = "UMAP", size = 0.2) + 
    scale_color_manual(values = metadata(spe)$color_vectors$patient_id) +
...
```{r visualizing-batch-correction-harmony-2, warning=FALSE, message=FALSE, fig.height=8, eval=FALSE}
# Before correction
plot_list <- multi_dittoDimPlot(spe, var = markers, reduction.use = "UMAP", 
                   assay = "exprs", size = 0.2, list.out = TRUE) 
plot_list <- lapply(plot_list, function(x) x + scale_color_viridis())
