...
The `Seurat` package provides a number of functionalities to analyse single-cell
...
correcting the expression values of cells.

To use `Seurat`, we will first create a `Seurat` object from `SpatialExperiment`
...
of each patient individually. The `FindIntegrationAnchors` detects MNNs between
...
considered for MNN detection. This increases the integration
strength.
...
Chosing the correct integration approach is challenging withou having ground truth
