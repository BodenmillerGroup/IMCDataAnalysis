...
reducedDim(spe, "corrected") <- reducedDim(out, "corrected")
```
...
markers <- c("Ecad", "CD45RO", "CD20", "CD3", "FOXP3", "CD206")
...
```{r batch-correction-harmony, message=FALSE, eval=FALSE}
library(harmony)
...
set.seed(220228)
spe <- runUMAP(spe, exprs_values = "harmony", name = "UMAP_harmony") 
```

```{r visualizing-batch-correction-harmony-1, message=FALSE}
library(cowplot)
library(dittoSeq)
library(viridis)
