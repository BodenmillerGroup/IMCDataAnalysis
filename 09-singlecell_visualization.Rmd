...
```{r cell type umap, fig.width=8, fig.height=8, message=FALSE}
library(dittoSeq)
library(scater)
library(patchwork)
library(cowplot)
...
```{r celltype violin, message=FALSE, fig.width=8, fig.height=7}
#Violin Plot - plotExpression
plotExpression(spe[,cur_cells], features = rownames(spe)[rowData(spe)$type_markers == TRUE],
...
In the following, we would like to highlight some useful visualization
functions from the
[CATALYST](https://bioconductor.org/packages/release/bioc/html/CATALYST.html)
package.
...
pbMDS(spe_cat, by = "cluster_id", 
      features = rownames(spe_cat)[rowData(spe_cat)$type_markers == TRUE], 
...
```{r celltype pbExprs, fig.width=10, fig.height=6}
...
Visualization of low-dimensional embeddings, here not corrected and
...
pbMDS(spe_cat, by = "sample_id", 
      color_by = "sample_id", features = rownames(spe_cat)[rowData(spe_cat)$type_markers == TRUE]) +
...
n_neighbors <- colPair(spe) %>% as.data.frame %>% group_by(from) %>% count() %>% arrange(desc(n))
