...
TODO I just noticed now that here you have a hyperlink for cytomapper but not for imcRtools. potentially you may want to be consistent. but just a minor thing.
...
TODO not sure I understand why this approach should be sensitive to lateral spillover. I see it more as an unbiased way to cluster cells based on neighborhood expression without having to go through clustering or cell type definition first. potentially the differences observed here are also due to the fact that knn should be smaller for expression based aggregation. It would also be nice to maybe show here a heatmap of aggregated expression instead of cell types.
...
an empirical null distribution which is generated by permuting all cell labels (while maintaining the tissue strcuture).
