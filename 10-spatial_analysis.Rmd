...
CN 9 and CN 4 are mainly composed of tumor cells with CN4 forming the tumor/stroma border.
CN 2 and CN 10 are mainly composed of B and BnT cells indicating TLS. CN 3 is composed 
of aggregated plasma cells and CN 7 contains most T cells.
...
**Of note:** constructing a 20-nearest neighbor graphs and clustering using
kmeans with `k=10` is only an example. Similar to the analysis done in 
Section \@ref(snn-graph), it is recommended to perform a parameter sweep
across different graph construction algorithms and different parmaters `k`
