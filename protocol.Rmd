...
```{r, include=FALSE}
p1 + p2
ggsave("outputs/Fig12_celltype-UMAP.pdf", p1, width = 6, height = 4)
ggsave("outputs/Fig12_cluster-UMAP.pdf", p2, width = 6, height = 4)
```
...
p <- plotSpatial(spe[,spe$celltype == "Tumor"], 
    ...
    theme(legend.position = "none") +
    scale_color_manual(values = rev(colors()))
ggsave("outputs/Fig14_spatial-communities.pdf", p)
```
...
p <- plotSpatial(spe, 
            ...
            node_size_fix = 0.5) +
    scale_color_brewer(palette = "Set3")
ggsave("outputs/Fig15_cellular-neighborhoods.pdf", p)
...
p <- plotSpatial(spe, 
            ...
            node_size_fix = 0.1, 
            colPairName = "knn_spatialcontext_graph")
ggsave("outputs/Fig16_spatial-contexts.pdf", p)
...
p <- plotSpatialContext(spe,
                   ...
                   node_label_color_by = "n_cells") +
    scale_color_viridis()
ggsave("outputs/Fig16_spatial-context-graph.pdf", p, width = 6, height = 4)
```
...
p <- plotSpatial(spe,
    ...
    theme(legend.position = "none") +
    scale_color_manual(values = rev(colors()))
ggsave("outputs/Fig17_patch-detection.pdf", p)
```
...
p <- out %>% as_tibble() %>%
  ...
  high = muted("red")) +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("outputs/Fig18_interaction-testing.pdf", p, width = 5, height = 3)
```
