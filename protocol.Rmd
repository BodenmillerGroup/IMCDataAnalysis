...
```{bash preprocess-panel, engine.opts="--init-file /Users/nils/IMCDataAnalysis/.alias -i"}
...
```{bash preprocess-images, engine.opts="--init-file /Users/nils/IMCDataAnalysis/.alias -i"}
...
```{bash segment-cells, engine.opts="--init-file /Users/nils/IMCDataAnalysis/.alias -i"}
...
```{bash measure-intensities, engine.opts="--init-file /Users/nils/IMCDataAnalysis/.alias -i"}
...
```{bash measure-regionprops, engine.opts="--init-file /Users/nils/IMCDataAnalysis/.alias -i"}
...
```{bash measure-neighbors, engine.opts="--init-file /Users/nils/IMCDataAnalysis/.alias -i"}
...
pdf("outputs/plotSpotHeatmap.pdf")
plotSpotHeatmap(sce)
dev.off()
```
...
```{r, include=FALSE}
pdf("outputs/before_after_single-cell.pdf")
before + after
dev.off()
```
...
pdf("outputs/before_after_images.pdf")
plotPixels(images[5], colour_by = "Yb173Di", 
           image_title = list(text = "Yb173 (Ecad) - before", 
                       position = "topleft"), 
           legend = NULL, bcg = list(Yb173Di = c(0, 4, 1)))
...
```{r, include=FALSE}
pdf("outputs/segmentation-quality.pdf")
plotPixels(cur_images,
           mask = masks[img_ids],
           img_id = "sample_id",
           missing_colour = "white",
...
```{r, include=FALSE}
pdf("outputs/marker-distribution.pdf")
multi_dittoPlot(spe, vars = c("HLADR", "CD3", "Ecad", "PDGFRb"),
               group.by = "patient_id", plots = c("ridgeplot"), 
               assay = "exprs")
dev.off()
...
```{r, include=FALSE}
pdf("outputs/UMAP_corrected.pdf")
dittoDimPlot(spe, var = "patient_id", 
                   reduction.use = "UMAP_mnnCorrected", size = 0.2) + 
    ggtitle("Patient ID on UMAP after correction")
dev.off()
...
sil <- vapply(as.list(combinations$clusters), 
              function(x) mean(approxSilhouette(mat[sam_cells,],
    x)$width), 0)
