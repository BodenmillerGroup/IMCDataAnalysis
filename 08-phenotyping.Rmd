...
```{r raw-counts, message=FALSE, results='hide', fig.show='hide'}
library(patchwork)
library(pheatmap)
library(gridExtra)
...
`scran` show similar results (left heatmap above). For example, Rphenograph
cluster 19 (a tumor cluster) is perfectly captured by SNN cluster 16. On the
other hand, the CD4 T cells cluster (SNN cluster 12) is split into Rphenograph
cluster 9, Rphenograph cluster 15 and Rphenograph cluster 17. A common approach
...
```{r corrected, message=FALSE, results='hide', fig.show='hide'}
...
infiltrated regions measured by IMC. We include this phenotype to account
for B cell/T cell interactions where precise classification into B cells or
T cells is not possible. The exact gating scheme can be seen at [TODO]().

As a related approach, [Astir](TODO) and [Garnet](TODO) use pre-defined panel information
to classify cell phenotypes based on their marker expression.
...
The labelled cells for this data set can be accessed at [TODO](TODO) and were
downloaded in Section \@ref(prerequisites). Per image, the `cytomapperShiny`
function allows the export of gated cells in form of `SingleCellExperiment` or
`SpatialExperiment` objects. The cell label is stored in
...
### Read in and consolidate labelled data

Here, we will read in the individual `SpatialExperiment` objects containing the
labelled cells and concatenate them. In the process of concatenating the 
`SpatialExperiment` objects along their columns, the `sample_id` entry is 
appended by `.1, .2, .3, ...` due to replicated entries. The last row in this
code chunk sets the `sample_id` entry back to its original entry.

```{r, read-in-labelled-data, message=FALSE}
library(SingleCellExperiment)
label_files <- list.files("/Volumes/projects/IMCWorkflow/Robjects/gated_cells/", 
...
In the next section we will identify cells that were labeled multiple times.
This occurs when different cell phenotypes are gated per image and in this 
dataset affects immune cells that are located inside the tumor compartment.

We will first identify those cells that were uniquely labelled and assign their
label. In the next step, we will identify those cells that were labelled twice
AND were labelled as Tumor cells. These cells will be assigned their immune cell
...
spe_labels <- rep("unlabelled", ncol(spe))
names(spe_labels) <- colnames(spe)
spe_labels[names(cur_labels)] <- cur_labels
spe$cell_labels <- spe_labels

# Number of cells labelled per patient
table(spe$cell_labels, spe$patient_id)
```

Based on these labels, we can now train a random forest classifier to classify
all remaining, unlabelled cells.

## Train classifier

In this section, we will use the [caret](TODO) framework for machine learning
in R. This package provides an interface to train a number of regression and
classification models in a coherent fashion. We use a random forest classifier
due to low number of parameters, speed and an observed high performance for
cell type classification [@Hoch2022].

In the following section, we will first split the `SpatialExperiment` object
into labelled and unlabelled cells. Based on the labelled cells, we split
the data into a train (75% of the data) and test (25% of the data) dataset. 
We currently do not provide an independently labelled validation dataset.
...
the best parameter, we will perform a 5-fold cross validation (se within
...
# Split between labelled and unlabelled cells
lab_spe <- spe[,spe$cell_labels != "unlabelled"]
unlab_spe <- spe[,spe$cell_labels == "unlabelled"]
...
To assess the accuracy, sensitivty, specificity, among other quality measures of
...
versus the true negative rate (1 - specificity):
...
data.frame(cm$byClass) %>%
  ...
  geom_point(aes(1 - Specificity, Sensitivity, 
                 ...
                 fill = class),
             shape = 21) + 
  scale_fill_brewer(10, palette = "Set3") +
  theme_classic(base_size = 15) + 
  ylab("Sensitivity (TPR)") +
  xlab("1 - Specificity (FPR)")
```

We observe TODO... The sice of the circle specifies the number of cells per class.
...
classifier to predict the cell phenotypes of the unlabelled data.

First, we predict the cell phenotypes and extract their classification
probabilities.

```{r, unlabelled-prediction}
...
cell_labels.class[rowMax(as.matrix(cell_labels.prob)) < 0.4] <- "undefined"

# Store labels in SpatialExperiment onject
cell_labels <- spe$cell_labels
cell_labels[colnames(unlab_spe)] <- cell_labels.class
...
```{r compare-raw, message=FALSE, results='hide', fig.show='hide'}
tab1 <- table(spe$celltypes, 
              paste("Rphenograph", spe$pg_clusters))
tab2 <- table(spe$celltypes, 
              paste("SNN", spe$nn_clusters))
tab3 <- table(spe$celltypes, 
              paste("SOM", spe$som_clusters))
...
We next compare cell classification against clustering results using the 
integrated cells.

```{r compare-corrected, message=FALSE, results='hide', fig.show='hide'}
tab1 <- table(spe$celltypes, 
...
We observe a high agreement between the shared nearest neighbor clustering approach using the 
integrated cells and the cell phenotypes derived by classification.

In the next sections, we will highlight visualization strategies to verify the 
correctness of the phenotyping approach.
...
test_that("images are written out", {
    skip_on_ci()
    cell_types <- unique(spe$celltypes)
    ...
    for(j in unique(spe$celltypes)){
        cur_sce <- spe[,spe$celltypes == j]
