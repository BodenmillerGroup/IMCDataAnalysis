...
spe$celltypes <- cell_labels 

cur_probs <- matrix(NA, ncol = ncol(cell_labels.prob), nrow = ncol(spe))
rownames(cur_probs) <- colnames(spe)
colnames(cur_probs) <- colnames(cell_labels.prob)
cur_probs[rownames(cell_labels.prob),] <- as.matrix(cell_labels.prob)

spe$probabilities <- DataFrame(cur_probs)
```

Visualization in next sections
