...
```{r raw-counts, message=FALSE, fig.width=25, fig.height=7}
library(patchwork)
library(pheatmap)
library(gridExtra)
...
We observe that `Rphenograph` and the shared nearest neighbor (SNN) approach by `scran`
show similar results (left heatmap above). For example, Rphenograph cluster 20 (a tumor cluster)
is perfectly captured by SNN cluster 15. On the other hand, the B cell cluster (Rphenograph cluster 19)
is split into SNN cluster 13 and SNN cluster 11. A common approach is to now merge clusters that contain
similar cell types and annotate them by hand.

Below, a comparison between the clustering results of the integrated cells 
is shown.

```{r corrected, message=FALSE, fig.width=25, fig.height=7}
...
for brevity we only highlighted the use of the silouhette width but different
