...
detect connected communities and partition the graph into clusters of cells. This
clustering strategy was used by Jackson, Fischer _et al._ to
cluster IMC data [@Jackson2020].
TODO you could add the Schulz et al publication from 2017 for Rphenograph as well :)
...
library(scran)
#TODO in the parameter sweep you use SNNgraph and here and below you use NNgraph. Is that on purpose, does it matter much?
...
times. This occurs when different cells phenotypes are gated per image and can
...
dittoHeatmap(spe[,cur_cells], 
             ...
             assay = "exprs", scale = "none",
             heatmap.colors = viridis(100), 
             annot.by = c("celltype", "patient_id"),
             annot.colors = c(dittoColors(1)[1:length(unique(spe$celltype))],
                              metadata(spe)$color_vectors$patient_id))
```
