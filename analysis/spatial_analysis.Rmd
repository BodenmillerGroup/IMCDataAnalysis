...
cur_tab <- proportions(x = table(sce$clustered_neighborhood, sce$CellType), 
                     margin = 1)

pheatmap(cur_tab, color = colorRampPalette(c("dark blue", "white", "dark red"))(100), 
         annotation_row = data.frame(row.names = rownames(cur_tab),
                                     clust = rownames(cur_tab)),
         annotation_colors = list(clust = col_vector), scale = "column")
```

And visualize the patches on the tissue.

```{r visualize-patches, , fig.width=10, fig.height=10}
