...
channels are indicated as 1, cytoplasmic channels are indicated as 2, and the
...
Observe the number of correctly and incorrectly assigned pixels per spot. The
following heatmap depicts the number of pixels assigned to each spot on the
...
Here, we use the `fastMNN` method of the 
...
CRITICAL: The performance of batch correction needs to be visually assessed to
...
expressed in cells that cluster in the UMAP embedding.
...
validation to tune the random forest model parameter.
...
Finally, these can be spatially visualized and the cell type fraction per CN can
be computed.
...
interactions as a layered graph.
...
constructs a convex hull around each component, and expands this hull to include
neighboring cells. Below, we detect connected tumor components made up of at
least 10 cells, and we slightly expand the convex hull to include cells within
the patch.
...
created using `steinbock`), the `testInteraction` function of the `imcRtools`
