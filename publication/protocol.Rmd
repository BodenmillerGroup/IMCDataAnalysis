...
```{r, include=FALSE}
pdf("outputs/Fig5_filterPixels.pdf", width = 7, height = 6)
pheatmap(log10(cur_table + 1),
         cluster_rows = FALSE,
         cluster_cols = FALSE)
dev.off()
...
```{r, message=FALSE}
cluster_celltypes <- recode(spe$nn_clusters,
...
spe$cluster_celltypes <- cluster_celltypes
```
...
```{r predict-cells}
# Select the asinh-transformed counts for prediction
...
p2 <- dittoDimPlot(spe, var = "cluster_celltypes", 
             reduction.use = "UMAP_mnnCorrected", size = 0.2,
             do.label = TRUE) +
